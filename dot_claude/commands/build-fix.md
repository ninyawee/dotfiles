...
|-----------|---------------|
| `package.json` with `build` script | `bun run build` |
| `tsconfig.json` (TypeScript only) | `bun x tsc --noEmit` |
| `Cargo.toml` | `cargo build 2>&1` |
| `pom.xml` | `mvn compile` |
| `build.gradle` | `./gradlew compileJava` |
| `go.mod` | `go build ./...` |
| `pyproject.toml` | `uv run mypy .` |
...
- Build errors stem from **missing dependencies** (need `bun install`, `cargo add`, `uv add`, etc.)
